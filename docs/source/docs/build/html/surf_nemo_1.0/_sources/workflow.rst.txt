...
.. _fig-main:
.. figure:: _static/figure/fig_workflow/work_flow_SURF_S.png
   :width: 60%

   Work-flow of the Relocatable SURF-NEMO platform.

The schematic work-flow diagrams in figure :numref:`fig-main` shown the steps executed by the SURF-NEMO
...
.. _fig-workflow:
.. figure:: _static/figure/fig_workflow/work_flow.png
   :width: 60%

   Graphical calling function flow of the Relocatable SURF-NEMO platform.

The graphical calling function flow (Figure :numref:`fig-workflow`) is a representation, using graph notation, of all paths that might be traversed through a program during its execution

We can identify 4 macrotask: Child meshmask generation, Atmospheric data regridding, Ocean IC data regridding, Ocean BC data regridding and OBC data extraction and Ocean Model simulation.

The dependency graph is show in Fugure :numref:`fig-workflow`. The dependency graph is a directed graph representing dependencies of several objects towards each other....
...
specify the value of the input simulation parameters for the ocean model in the configuration file

The first phase execute by each macrotask consists in setting up the input model parameters.
The input parameters are described in chapter 3 and appendix A.
The model parameters are specifyin the configuration file (json format) and the procedures executed are ``read_inJsonFree`` and ``read_inJsonFixed``
...
In this phase the procedure ``set_pathData`` ``set_fileData`` are also call to define all the paths and files name
used by the program for the specific experiment to carried out.
...
the edges of the grid volumes and the relative vorticity (:math:`\rho`) and planetary vorticity (:math:`\rho`) are defined in the
...
is executed. If some of the requested data are not present then the procedures downlCoastlineInfile,
...
of the first row and first column of T grid. On the xx plane, the location of the T point of the grid are:
